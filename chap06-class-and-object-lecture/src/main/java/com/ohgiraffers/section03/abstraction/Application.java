...
public class Application {
    public static void main(String[] args){
        ...
        * 프로그램을 만들 때 이용하여 새로운 세계를창조하는 방법론이다.
        ...
        * 따라서 객체를 설계할 때는 속성은 부가적이며 적절한 책임을 가진 객체들간의 주고 밥는 메세지를 이용해 상호작용하는 것을
        * 설계하는 것을 의미한다.
        * 즉, 필드보다는 메소드를 중점적으로 추상화  기법을 적용하여 객체를 설계하고, 그 객체를 구현하는 방법을로 설계한 대로 클래스를
        * 작성하는 것이다. 이것을 추상화라고 한다.
        * */


        /*
        * 프로그램 설계하기
        * 자동차 운전 게임에서 카레이서가 자동차를 운전하다.
        * 필요한 객체를 도출한다.
        * - 플레이어(사용자), 카레이서, 자동차
        *
        * 객체 간의 상호작용
        ...
        * 커뮤니케이션 다이어그램
        * 1. 시동 걸기
        * 사용자 ----> 시동을 걸어라  ----> |카레이서| ----> 시동을 걸어라 ----> |자동차| ----> 시동걸기
        *
        * 2. 가속
        * 사용자 ----> 엑셀을 밟아라 -----> |카레이서| -----> 앞으로 가라 -----> |자동차| ----> 앞으로 간다
        *
        * 3. 멈추기
        * 사용자 ----> 멈추어라 ----> |카레이서| -----> 브레이크 밝기 -----> |자동차| ----> 멈추기
        *
        * 4. 시동끄기
        * 사용자 ----> 시동을 꺼라 ------> |카레이서| ------> 시동끄기 ------> |자동차| -----> 시동끄기
        *
        * 클래스 설계하기
        * 카레이서 클래스
        * 1. 속성 : 자동차
        ...
        * */
        ...
        loop : while(true){
            ...
            switch (no){
                ...
                case 9 : System.out.println("프로그램을 종료합니다."); break loop; // loop를 멈춘다. 넣지 않으면 switch를 멈춘다.
                default:
                    System.out.println("잘못된 버튼을 선택하셨습니다. 다시 선택해주세요."); break;

            }

//            if( no == 9){
//                break; loop를 설정하지 않은 경우
        }
    }
    ...
    * 복잡한 현실세계의 개념, 데이터, 시스템 또는 프로세스를 단순화하거나 요약하여 핵심적인 특징을 강조하고 불필요한 세부 사항을 숨기는 과정이다.
    *
    ...
    *
    * 은닉화 : 불필요한 세부사항을 숨기고 필요한 정보만을 노출한다. 이것은 정보 은닉과 관련이 있으며 객체의 내부 구형을 외부로부터 숨겨 보호하고
